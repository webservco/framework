...
final class ArrayStorage
{
     ...
     */
    public static function add(array $storage, mixed $setting, mixed $data): array
    {
     ...
     * @throws \WebServCo\Framework\Exceptions\ArrayStorageException
     */
    public static function append(array $storage, mixed $data = []): array
    {
     ...
     *                          (eg 'i18n/lang').
     */
    public static function get(mixed $storage, mixed $setting = null, mixed $defaultValue = null): mixed
    {
        $setting = self::parseSetting($setting);
     ...
     *                          (eg 'i18n/lang').
     */
    public static function getElse(mixed $storage, mixed $setting = null, mixed $defaultValue = null): mixed
    {
    ...
    *                          (eg 'i18n/lang').
    */
    public static function has(mixed $storage, mixed $setting): bool
    {
        $value = 'WSFW_NOEXIST';
     ...
     * @throws \WebServCo\Framework\Exceptions\ArrayStorageException
     */
    public static function remove(array $storage, mixed $setting): array
    {
        if (!is_array($storage) || empty($setting)) {
            throw new ArrayStorageException('Invalid parameters specified.');
        }
     ...
     * @throws \WebServCo\Framework\Exceptions\ArrayStorageException
     */
    public static function set(array $storage, mixed $setting, mixed $value): array
    {
        if (!is_array($storage) || empty($setting)) {
            throw new ArrayStorageException('Invalid parameters specified.');
        }
     ...
     * or an array.
     */
    private static function parseSetting(mixed $setting): mixed
    {
        if (is_string($setting) && strpos($setting, Settings::DIVIDER) !== false) {
            return explode(Settings::DIVIDER, $setting);
        }
